...
int fuel_to_move(int start, int end) {
	// last_move cost is  max(start,end) - min(start, end)
	// first_move cost is 1
	// arithmetic series
	// last_move + first_move times number of moves /2
	return (1 + std::max(start,end) - std::min(start,end)) * abs(start - end) / 2;
}


int total_fuel_needed(int position, const std::vector<int>& data) {
	int sum{ 0 };
	for (const int& i : data) sum += fuel_to_move(i, position);
	return sum;
}
