...
int manhattan_distance(const Scanner& s1, const Scanner& s2) {
	return abs(s1.x() - s2.x()) + abs(s1.y() - s2.y()) + abs(s1.z() - s2.z());
}

int max_manhattan(const std::vector<Scanner>& scanners) {
	int result{ 0 };
	for (int i = 0; i < scanners.size(); i++) {
		for (int j = i + 1; j < scanners.size(); j++) {
			result = std::max(result, manhattan_distance(scanners[i], scanners[j]));
		}
	}
	return result;
}
...
int main() {
	...
	std::cout << "Maximum manhattan distance between scanners is " << max_manhattan(scanners) << "." << std::endl;
}